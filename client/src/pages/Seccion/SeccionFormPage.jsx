...
import { createSeccion, deleteSeccion, updateSeccion, getSeccion } from "../../api/asistencia.api"
...
export function SeccionFormPage() {
    ...
    const onSubmit = handleSubmit(async data => {
        if (params.id) {
            await updateSeccion(params.id, data)
        } else {
    ...
    useEffect(() => {
        async function loadSeccion() {
            if (params.id) {
                const res = await getSeccion(params.id)
                setValue('codigo_seccion', res.data.codigo_seccion)
            }
        }
        loadSeccion()
    }, [])
            ...
            <div className="">
                <form className="flex flex-col gap-2 px-80 py-2" onSubmit={onSubmit}>
                    <label className="flex justify-center items-center gap-2 py-2">
                        ...
                        <span className="text-xl border-b-2 border-blue-800">Crear sección</span>
                    </label>

                    <label className="flex justify-start items-center gap-2" htmlFor="codigo_seccion">
                        <span className="text-lg">Código de sección</span>
                        ...
                        { errors.codigo_seccion && <li className="text-red-600">El código de sección es requerido.</li> }
                    </label>
                    <input className="border-2 border-black rounded-md p-2" type="text" {...register('codigo_seccion', {required: true})} />
