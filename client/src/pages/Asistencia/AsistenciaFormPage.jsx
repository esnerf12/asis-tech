...
import { createAsistencia, deleteAsistencia, updateAsistencia, getAsistencia, getAllClases, getAllEstudiantes } from "../../api/asistencia.api"
...
export function AsistenciaFormPage() {
    ...
    const onSubmit = handleSubmit(async data => {
        if (params.id) {
            await updateAsistencia(params.id, data)
        } else {
    ...
    useEffect(() => {
        async function loadAsistencia() {
            if (params.id) {
                const res = await getAsistencia(params.id)
                setValue('clase_id', res.data.clase_id)
                setValue('estudiante_id', res.data.estudiante_id)
            }
        }
        loadAsistencia()
    }, [])
            ...
            <div className="">
                <form className="flex flex-col gap-2 px-80 py-2" onSubmit={onSubmit}>
                    <label className="flex justify-center items-center gap-2 py-2">
                        ...
                        <span className="text-xl border-b-2 border-blue-800">Crear asistencia</span>
                    </label>
                    <label className="flex justify-start items-center gap-2" htmlFor="clase_id">
                        ...
                        { errors.clase_id && <li className="text-red-600">La clase es requerida.</li> }
                    </label>
                    <select className="border-2 border-black rounded-md p-2" name="clase_id" id="clase_id" {...register('clase_id', {required: true})}>
                        {
                            clase && clase.map(cls => (
                    ...
                    <label className="flex justify-start items-center gap-2" htmlFor="estudiante_id">
                        <span className="text-lg">Estudiante</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-info-circle text-blue-900 cursor-pointer" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                            <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/>
                        </svg>
                        { errors.estudiante_id && <li className="text-red-600">El estudiante es requerido.</li> }
                    </label>
                    <select className="border-2 border-black rounded-md p-2" name="estudiante_id" id="estudiante_id" {...register('estudiante_id', {required: true})}>
                        {
                            estudiante && estudiante.map(est => (
                                <option key={est.id} value={est.id}>{est.persona_id}</option>
                            ))
                        }
                    </select>
