...
import { createTipoHorario, deleteTipoHorario, updateTipoHorario, getTipoHorario } from "../../api/asistencia.api"
import { useForm } from "react-hook-form"
import { useNavigate, useParams } from "react-router-dom"


export function TipoHorarioFormPage() {
    ...
    const onSubmit = handleSubmit(async data => {
        if (params.id) {
            await updateTipoHorario(params.id, data)
        } else {
            await createTipoHorario(data)
        }
        navigate('/tipo_horario')
    })

    useEffect(() => {
        async function loadTipoHorario() {
            if (params.id) {
                const res = await getTipoHorario(params.id)
            ...
            <div className="">
                <form className="flex flex-col gap-2 px-80 py-2" onSubmit={onSubmit}>
                    <label className="flex justify-center items-center gap-2 py-2">
                        ...
                        <span className="text-xl border-b-2 border-blue-800">Crear tipo de horario</span>
                    </label>

                    <label className="flex justify-start items-center gap-2" htmlFor="tipo">
                        ...
                        { errors.tipo && <li className="text-red-600">El tipo es requerido.</li> }
                    </label>
                    <input className="border-2 border-black rounded-md p-2" type="text" {...register('tipo', {required: true})} />
                    ...
                    { params.id && <button onClick={async() => {
                        ...
                        if (accepted) {
                            await deleteTipoHorario(params.id)
                            navigate('/tipo_horario')
                        }
