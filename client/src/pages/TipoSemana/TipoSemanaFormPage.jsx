...
import { createTipoSemana, deleteTipoSemana, updateTipoSemana, getTipoSemana } from "../../api/asistencia.api"
import { useForm } from "react-hook-form"
import { useNavigate, useParams } from "react-router-dom"

export function TipoSemanaFormPage() {
    ...
    const onSubmit = handleSubmit(async data => {
        ...
        navigate('/tipo_semana')
    })

    useEffect(() => {
        async function loadTipoSemana() {
            if (params.id) {
                const res = await getTipoSemana(params.id)
            ...
            <div className="">
                <form className="flex flex-col gap-2 px-80 py-2" onSubmit={onSubmit}>
                    <label className="flex justify-center items-center gap-2 py-2">
                        ...
                        <span className="text-xl border-b-2 border-blue-800">Crear tipo de semana</span>
                    </label>
                    ...
                    { params.id && <button onClick={async() => {
                        ...
                        if (accepted) {
                            await deleteTipoSemana(params.id)
                            navigate('/tipo_semana')
                        }
